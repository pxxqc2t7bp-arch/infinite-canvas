...
- 用户运行 `npm exec --yes --package=@pxxqc2t7bp-arch/canvas-agent@0.7.1 -- canvas-agent` 后，线上画布能显示已连接。
...
当前架构进一步支持 Codex、ZCode、TraeCode 等多个外部 MCP 客户端同时连接：

- 每个 MCP 进程通过 `--agent`、`--name` 和可选 `--instance` 声明独立身份。
- 画布读取返回 `projectId` 与单调递增的 `revision`；写操作必须携带读取版本。
- 不同节点的并发修改自动合并，同一节点或失效项目的写入返回结构化冲突，Agent 重读后重试。
- 用户手动修改、插件操作和生成结果同样进入 revision journal，不会被旧 Agent 请求静默覆盖。
- 多个待确认画布写入进入 FIFO 队列，并在“协作”页显示在线 Agent 和最近活动。

跨产品调度使用持久化任务邮箱。`broadcast` 会让多个目标 Agent 分别领取同一任务并提交结果；`orchestrated` 允许主 Agent 创建带父子关系和依赖的任务。外部 MCP 客户端必须主动轮询任务，服务端不能通过 MCP 反向唤醒它们。Codex 还可显式选择 app-server 主动调度，默认与 ZCode、TraeCode 一样使用任务邮箱。
