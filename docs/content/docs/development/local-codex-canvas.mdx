...
- Browser connections require the generated token, and each authenticated origin is added to the local allowlist. This browser CORS allowlist does not limit local MCP clients.
- External Agent identities provide local attribution and coordination rather than separate authentication. Local processes that can read the same connect token share one trust boundary.
