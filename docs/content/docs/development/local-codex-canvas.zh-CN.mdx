...
- 网页带正确 token 连接后，`Canvas Agent` 会把该网页 Origin 加入本机白名单；该白名单只约束浏览器跨域请求，不限制本机 MCP 客户端数量。
- 外部 Agent 的身份用于本机协作归属和界面展示，不是独立认证凭据；能够读取同一个 Connect token 的本机进程处于同一信任边界。
