...
```bash
npm exec --yes --package=@pxxqc2t7bp-arch/canvas-agent@0.7.1 -- canvas-agent
```

需要排查连接、线程、Codex app-server 或工具调用问题时，可开启 Debug 模式：

```bash
npm exec --yes --package=@pxxqc2t7bp-arch/canvas-agent@0.7.1 -- canvas-agent --debug
```
...
直接运行 `npm exec --yes --package=@pxxqc2t7bp-arch/canvas-agent@0.7.1 -- canvas-agent` 只启动本地 Agent 服务，不会安装 MCP，也不会增加 Codex 工具上下文。只有安装 Codex app 插件，或手动执行 `codex mcp add` 后，`infinite-canvas` 工具才会进入 Codex 上下文；由于工具较多，不使用时建议移除。
...
```bash
npm exec --yes --package=@pxxqc2t7bp-arch/canvas-agent@0.7.1 -- canvas-agent mcp --agent codex --name Codex
```
...
```bash
codex mcp add infinite-canvas -- npm exec --yes --package=@pxxqc2t7bp-arch/canvas-agent@0.7.1 -- canvas-agent mcp --agent codex --name Codex
```
...
command = "npm"
args = ["exec", "--yes", "--package=@pxxqc2t7bp-arch/canvas-agent@0.7.1", "--", "canvas-agent", "mcp", "--agent", "codex", "--name", "Codex"]
...
```bash
claude mcp add --scope user --transport stdio infinite-canvas -- npm exec --yes --package=@pxxqc2t7bp-arch/canvas-agent@0.7.1 -- canvas-agent mcp --agent claude --name "Claude Code"
```
