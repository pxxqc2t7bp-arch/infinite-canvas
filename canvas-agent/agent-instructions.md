...
- 每次修改画布前必须先调用 `canvas_get_state`，并把返回的 `projectId` 和 `revision` 原样作为写工具的 `projectId` 与 `baseRevision`。遇到 `CANVAS_REVISION_CONFLICT` 或 `CANVAS_REVISION_EXPIRED` 时重新读取，只基于最新状态重试，不得猜测版本或覆盖冲突内容。
...
- 参与多 Agent 协作时先调用 `collaboration_join`，再用 `collaboration_list_tasks` 和 `collaboration_claim_task` 主动领取任务；MCP 服务不会反向唤醒客户端。完成后用 `collaboration_submit_result` 提交，失败时用 `collaboration_fail_task`，不要把协作结果直接当成已经写入画布。
