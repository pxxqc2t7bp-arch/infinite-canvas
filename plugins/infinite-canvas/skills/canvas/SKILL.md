...
- 所有画布写工具都必须携带最近一次读取返回的 `projectId` 和 `revision`（作为 `baseRevision`）。遇到版本冲突时重新读取后再重试，不要覆盖其他 Agent 或用户刚完成的修改。
...
- 参与协作任务时先加入会话，再主动列出并领取任务；完成或失败后必须提交结果。MCP 服务不会反向唤醒外部 Agent。
